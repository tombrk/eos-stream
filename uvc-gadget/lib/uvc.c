...
uvc_events_process_control(struct uvc_device *dev, uint8_t req, uint8_t cs, uint8_t len,
			   struct uvc_request_data *resp)
{
	printf("control request (req %s cs %s len %u)\n", uvc_request_name(req), pu_control_name(cs), len);
	(void)dev;

	/*
	 * Return proper values for processing unit controls so that
	 * strict UVC hosts (e.g. Safari/WebKit) don't crash.
	 */
	switch (req) {
	case UVC_GET_INFO:
		/* Bit 0: GET supported, Bit 1: SET supported */
		resp->data[0] = 0x03;
		resp->length = 1;
		break;
	case UVC_GET_CUR:
	case UVC_GET_DEF:
		/* Return 0 as current/default value */
	...
	case UVC_GET_MIN:
		...
		break;
	case UVC_GET_MAX:
		/* Return 255 (or 0xffff for 2-byte) as maximum */
		if (len == 1) {
			resp->data[0] = 0xff;
		} else if (len >= 2) {
			resp->data[0] = 0xff;
			resp->data[1] = 0x7f; /* 32767 signed */
		}
		resp->length = len;
		break;
	case UVC_GET_RES:
